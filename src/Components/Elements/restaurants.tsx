...
const Restaurants = ({restaurants, limit}: Restaurants ) => {
  ...
  const mappedRestaurants  = allRestaurants?.map( (restaurant: Restaurant) => {
    ...
    const handleCardLink = () => {
        ...
        .then( (res:any) => {
          dispatch(singleRestaurantAction(res))
  ...
  return (
    <Grid container spacing={4}>
      {mappedRestaurants.length > 0 ? mappedRestaurants : <NoResults errorText="Nu s-a gasit niciun rezultat pentru restaurantul dorit, mai incercati" /> }
    </Grid>
  )
}
